...
- **Azure** — _coming soon_
- **Google Cloud Platform (GCP)** — _coming soon_
...
| `Accounts[]`      | `aws.accounts`          | Explicit list of AWS account IDs to discover.                                                                                                    | Optional.                                                                                                                                         |
| `AssumeRole`      | `aws.assume_role`       | IAM role name assumed in each target account.                                                                                                    | **Required** when `list_all_accounts` is `true` or `accounts` are provided. The connector assumes `arn:aws:iam::<account-id>:role/<assume_role>`. |
